...
// FC가 업무를 지시하는 역할을 하는 일반 자바 클래스(POJO)
// -> 해당하는 파일을 통해서만, jsp or html문서를 화면에 띄울 수 있다.
@Controller 
//-> Spring Contrainer가 해당하는 파일을 scan할 수 있게끔 해주는 annotation
...
public class BoardController {
	...
	public String insert(Board board) {
		...
		//2. board.jsp로 이동 (화면에 추가한 게시글이 보여야 함)
		return "redirect:/";
	}
	...
	@GetMapping("/count")
	@ResponseBody // --> 페이지를 이동하는 메소드가 아니라, 비동기통신을 처리하는 메스드임을 나타내는 annotation이다 
	...
	public String temp(RedirectAttributes rttr) {
		// redirect로 이동했을 때, 마지막 이동하는 페이지에 수집한 데이터를 전송하고 싶다면 rttr 사용가능
		rttr.addAttribute("temp","temp");
		// rttr의 경우에는 Model interface를 구현하고 있기 때문에
		// 2번째로 저장할 수 있는 매개변수가 Object라고 작성은 되어있으나 문자열만 저장 가능하다.
